...
<div class="row container" id="wrapper">
      <div class="halim-panel-filter">
         <div class="panel-heading">
            <div class="row">
               <div class="col-xs-6">
                  <div class="yoast_breadcrumb hidden-xs"><span><span><a href="">{{$phim->theloai->title}}</a> » <span>
                     <a href="{{route('quocgia',[$phim->quocgia->slug])}}">{{$phim->quocgia->title}}</a> »
                     <a href="{{route('namphim',$phim->year)}}">{{$phim -> year}}</a> »
                     <span class="breadcrumb_last" aria-current="page">{{$phim->title}}</span></span></span></span></div>
               </div>
            </div>
         </div>
            ...
            <div class="clearfix wrap-content">
               ...
               <div class="title-block">
                  ...
                  <div class="title-wrapper-xem full">
                     {{-- <h1 class="entry-title"> --}}
                        <a href="" title=" {{$phim ->title}}" class="tl">
               ...
               <div id="halim-list-server">
                  <ul class="nav nav-tabs" role="tablist">
                     ...
                     @else
                            <li role="presentation" class="active server-1"><a href="#server-0" aria-controls="server-0" role="tab" data-toggle="tab">Thuyết minh</a></li>
                     @endif
                     ...
                     <div role="tabpanel" class="tab-pane active server-1" id="server-0">
                        <div class="halim-server">
                           <ul class="halim-list-eps">
                              @foreach ($phim->tapphim as $key => $sotap)
                                 <a href="{{url('xem-phim/'.$phim->slug.'/tap-'.$sotap->tap)}}">
                                    <li class="halim-episode">
                                       <span class="halim-btn halim-btn-2 {{$tp == $sotap->tap ? 'active' : ''}} halim-info-1-1 box-shadow" data-post-id="37976" data-server="1" data-episode="1" data-position="first" data-embed="0"
                                          data-title="Xem phim {{$phim->title}} - Tập {{$sotap->tap}} - {{$phim->title_es}}"
                                          data-h1="{{$phim->title}} - tập {{$sotap->tap}}">
                                          {{$sotap->tap}}
                                       </span>
                                    </li>
                                 </a>
